...
export default function App(){
  ...
  return(
    <>
      ...
      {page==="about"&&(
        <>
          <PageBanner tag="ℹ️ À propos" title="Les Éclaireurs!" sub="La plateforme nationale de référence pour la montée en compétences de la filière électrique française."/>
          ...
          <section className="section">
            <div className="section-inner">
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"3rem",alignItems:"center"}}>
                <div>
                  <div className="s-chip" style={{background:"var(--blue-lt)",color:"var(--blue)",marginBottom:"1rem"}}>⚡ Notre mission</div>
                  <h2 style={{fontSize:"1.9rem",fontWeight:800,color:"var(--text)",lineHeight:1.2,marginBottom:"1.25rem"}}>
                    La technologie est prête.<br/>
                    <span style={{color:"var(--blue)"}}>Mais sans les compétences,<br/>la transition s'arrête.</span>
                  </h2>
                  <p style={{fontSize:"1rem",color:"var(--text2)",lineHeight:1.75,marginBottom:"1rem"}}>
                    Pour atteindre la neutralité carbone et limiter le réchauffement à 2°C, la part de l'électricité dans le mix énergétique mondial doit passer de <strong>20% à 50% d'ici 2050</strong>. C'est une opportunité majeure — mais aussi un défi colossal.
                  </p>
                  <p style={{fontSize:"1rem",color:"var(--text2)",lineHeight:1.75}}>
                    Sans compétences humaines adaptées, cette transition restera théorique. Les Éclaireurs! est né pour lever ce verrou : former, développer et mobiliser les talents capables de concevoir, déployer et exploiter les systèmes électriques de demain.
                  </p>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem"}}>
                  {[
                    {val:"760 000",lbl:"postes/an à pourvoir\n(2019–2030)",col:"var(--blue)"},
                    {val:"200 000",lbl:"emplois supplémentaires\nd'ici 2028",col:"var(--violet)"},
                    {val:"109+",lbl:"formations disponibles\nsur la plateforme",col:"var(--green)"},
                    {val:"3 352",lbl:"professionnels formés\ndepuis le lancement",col:"var(--orange)"},
                  ].map(({val,lbl,col})=>(
                    <div key={val} style={{background:"white",borderRadius:"var(--rl)",border:"1.5px solid var(--border)",padding:"1.25rem",boxShadow:"var(--sh)"}}>
                      <div style={{fontSize:"1.7rem",fontWeight:800,color:col,lineHeight:1,marginBottom:".4rem"}}>{val}</div>
                      <div style={{fontSize:".72rem",color:"var(--text2)",lineHeight:1.4,whiteSpace:"pre-line"}}>{lbl}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <section className="section" style={{background:"var(--bg2)"}}>
            <div className="section-inner">
              <div className="section-head">
                <div className="s-chip" style={{background:"var(--green-lt)",color:"var(--green)"}}>🎯 Pourquoi cette plateforme</div>
                <h2 className="s-title">Un hub neutre, multi-sources, agnostique marque</h2>
                <p className="s-desc">Les Éclaireurs! agrège et organise l'ensemble des ressources de formation de la filière électrique en un seul endroit accessible, gratuit et finançable.</p>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"1.5rem"}}>
                {[
                  {ico:"🔍",title:"Agrégation neutre",desc:"109+ formations issues de Schneider Electric, Legrand, Hager, Siemens, Rexel, Sonepar et organismes certifiants — réunies, organisées par compétences.",col:"var(--blue)"},
                  {ico:"🎯",title:"Parcours personnalisés",desc:"3 niveaux de maîtrise : Familiarisation, Solutions Tertiaires, Expertise & Numérique. Un parcours structuré pour chaque profil, du CAP à Bac+5.",col:"var(--violet)"},
                  {ico:"💳",title:"100% finançable",desc:"CPF, OPCO, aides régionales, dispositifs ADEME, CEE. Accompagnement dans toutes les démarches administratives inclus.",col:"var(--green)"},
                  {ico:"📱",title:"Mobile-first",desc:"Conçu pour l'électricien sur chantier. Accès hors-ligne, formats courts, tutos d'installation directement utilisables sur le terrain.",col:"var(--orange)"},
                  {ico:"💬",title:"Communauté active",desc:"Forum entre pairs, réseau d'électriciens, partage de bonnes pratiques. La compétence se construit aussi par l'échange.",col:"var(--blue)"},
                  {ico:"🌱",title:"Impact mesuré",desc:"142 tonnes de CO₂ évitées depuis le lancement. Chaque électricien formé accélère l'électrification et réduit les émissions.",col:"var(--green)"},
                ].map(({ico,title,desc,col})=>(
                  <div key={title} style={{background:"white",borderRadius:"var(--rl)",border:"1.5px solid var(--border)",padding:"1.5rem",boxShadow:"var(--sh)"}}>
                    <div style={{width:44,height:44,borderRadius:12,background:col+"15",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.4rem",marginBottom:"1rem"}}>{ico}</div>
                    <div style={{fontWeight:800,fontSize:".95rem",color:"var(--text)",marginBottom:".5rem"}}>{title}</div>
          ...
          <section className="section">
            <div className="section-inner">
              <div className="section-head">
                <div className="s-chip" style={{background:"var(--violet-lt)",color:"var(--violet)"}}>🤝 Gouvernance</div>
                <h2 className="s-title">Un Consortium Public-Privé au service de la filière</h2>
                <p className="s-desc">Les Éclaireurs! est portée par un écosystème d'industriels, de distributeurs et d'acteurs institutionnels réunis autour d'un objectif commun : sécuriser les compétences de la transition énergétique.</p>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"2rem"}}>
                <div style={{background:"var(--bg2)",borderRadius:"var(--rl)",padding:"2rem"}}>
                  <h3 style={{fontWeight:800,fontSize:1+".rem",color:"var(--text)",marginBottom:"1rem"}}>🏭 Industriels & Distributeurs</h3>
                  <div style={{display:"flex",flexWrap:"wrap",gap:".6rem"}}>
                    {["Schneider Electric","Legrand","Hager","Siemens","Rexel","Sonepar"].map(n=>(
                      <span key={n} style={{background:"white",border:"1.5px solid var(--border)",borderRadius:"var(--rf)",padding:"5px 14px",fontSize:".78rem",fontWeight:600,color:"var(--text)"}}>{n}</span>
                    ))}
                  </div>
                  <p style={{fontSize:".82rem",color:"var(--text2)",lineHeight:1.65,marginTop:"1rem"}}>Les leaders industriels de la filière co-investissent dans la plateforme et contribuent leurs contenus de formation. Leur rôle : être la locomotive de la montée en compétences de tout l'écosystème, y compris les PME et ETI.</p>
                </div>
                <div style={{background:"var(--bg2)",borderRadius:"var(--rl)",padding:"2rem"}}>
                  <h3 style={{fontWeight:800,fontSize:1+".rem",color:"var(--text)",marginBottom:"1rem"}}>🏛️ Partenaires institutionnels</h3>
                  <div style={{display:"flex",flexWrap:"wrap",gap:".6rem"}}>
                    {["ADEME","Enedis","EDF","RTE","France Compétences","Régions"].map(n=>(
                      <span key={n} style={{background:"white",border:"1.5px solid var(--border)",borderRadius:"var(--rf)",padding:"5px 14px",fontSize:".78rem",fontWeight:600,color:"var(--text)"}}>{n}</span>
                    ))}
                  </div>
                  <p style={{fontSize:".82rem",color:"var(--text2)",lineHeight:1.65,marginTop:"1rem"}}>Les acteurs publics apportent légitimité, financements (France 2030, AMI CMA) et ancrage territorial. La réussite de la transition énergétique dépend d'une coordination fine entre branches professionnelles, entreprises et opérateurs de compétences.</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="section" style={{background:"var(--bg2)"}}>
            <div className="section-inner">
              <div className="section-head">
                <div className="s-chip" style={{background:"var(--blue-lt)",color:"var(--blue)"}}>🗓️ Feuille de route</div>
                <h2 className="s-title">Un projet en deux temps</h2>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"2rem"}}>
                {[
                  {phase:"Phase 1",period:"0 – 6 mois",col:"var(--blue)",bg:"var(--blue-lt)",items:["Parcours pilotes et tests territoriaux","Clarification du modèle économique","Intégration des premiers partenaires","Lancement communauté électriciens"]},
                  {phase:"Phase 2",period:"6 – 18 mois",col:"var(--violet)",bg:"var(--violet-lt)",items:["Industrialisation et montée en charge","Intégration certifications CPF/OPCO","Massification du micro-learning","Extension aux lycées professionnels"]},
                ].map(({phase,period,col,bg,items})=>(
                  <div key={phase} style={{background:"white",borderRadius:"var(--rl)",border:`2px solid ${col}`,padding:"2rem",boxShadow:"var(--sh)"}}>
                    <div style={{display:"flex",alignItems:"center",gap:".75rem",marginBottom:"1.25rem"}}>
                      <div style={{background:bg,color:col,fontWeight:800,fontSize:".8rem",padding:"4px 14px",borderRadius:"var(--rf)"}}>{phase}</div>
                      <div style={{fontSize:".82rem",color:"var(--text2)"}}>{period}</div>
                    </div>
                    <ul style={{listStyle:"none",padding:0,margin:0,display:"flex",flexDirection:"column",gap:".65rem"}}>
                      {items.map(item=>(
                        <li key={item} style={{display:"flex",gap:".6rem",alignItems:"flex-start",fontSize:".85rem",color:"var(--text)"}}>
                          <span style={{color:col,fontWeight:800,flexShrink:0}}>→</span>{item}
                        </li>
                      ))}
                    </ul>
                  </div>
          ...
          <section className="section">
            <div className="section-inner">
              <div style={{background:"linear-gradient(135deg,var(--blue),var(--violet))",borderRadius:"var(--rl)",padding:"3rem",textAlign:"center",color:"white"}}>
                <div style={{fontSize:"2rem",marginBottom:".75rem"}}>🤝</div>
                <h2 style={{fontWeight:800,fontSize:"1.6rem",marginBottom:".75rem"}}>Rejoindre le consortium</h2>
                <p style={{fontSize:"1rem",opacity:.85,lineHeight:1.7,maxWidth:560,margin:"0 auto 2rem"}}>
                  Vous êtes un industriel, un organisme de formation, une région ou une fédération professionnelle ? Contribuez à faire des Éclaireurs! la référence nationale de la montée en compétences électriques.
                </p>
                <div style={{display:"flex",gap:"1rem",justifyContent:"center",flexWrap:"wrap"}}>
                  <button className="btn-primary" style={{background:"white",color:"var(--blue)"}} onClick={()=>nav("formations")}>Explorer la plateforme</button>
                  <button className="btn-secondary" style={{borderColor:"rgba(255,255,255,.4)",color:"white",background:"rgba(255,255,255,.12)"}} onClick={()=>nav("depot")}>Déposer un contenu</button>
                </div>
              </div>
            </div>
          </section>
      ...
      {page==="about"&&(
        <>
          <PageBanner tag="À propos" title="Notre mission" sub="Accélérer la transition énergétique en dotant chaque électricien des compétences du futur." showA11y/>
              ...
              <div style={{marginBottom:"4rem"}}>
                ...
                <div className="live-block">
                  <div style={{maxWidth:"700px",margin:"0 auto",textAlign:"center",position:"relative",zIndex:1}}>
                    <div className="s-chip" style={{background:"rgba(255,255,255,.2)",color:"white",margin:"0 auto .75rem"}}>Modèle de financement</div>
                    <h3 style={{fontWeight:800,fontSize:"1.4rem",color:"white",marginBottom:".75rem"}}>45% fonds privés · 55% subventions publiques</h3>
                    <p style={{fontSize:".88rem",color:"rgba(255,255,255,.82)",lineHeight:1.7}}>Conformément au modèle des campus d'excellence France 2030, la plateforme est cofinancée par les industriels du consortium et sécurisée par des enveloppes publiques sur 5 ans.</p>
                  </div>
                </div>
              </div>
